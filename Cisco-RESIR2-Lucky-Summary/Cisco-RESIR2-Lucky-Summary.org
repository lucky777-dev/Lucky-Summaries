...
- =line console 0= \textcolor{luckydarkgray}{(Accès à la première interface de la console de ligne, souvent la seule disponible)}
- =password azerty= \textcolor{luckydarkgray}{(Configuration du mot de passe "azerty")}
- =login= \textcolor{luckydarkgray}{(Activation de l'accès d'exécution utilisateur)}
...
- =configure terminal= \textcolor{luckydarkgray}{(Entrer dans le mode de configuration globale)}
- =enable secret azerty= \textcolor{luckydarkgray}{(Configuration du mot de passe "azerty")}
...
- =password azerty= \textcolor{luckydarkgray}{(Configuration du mot de passe "azerty")}
- =login= \textcolor{luckydarkgray}{(Activation de l'accès VTY)}
- =end= \textcolor{luckydarkgray}{(Quitter tous les modes de configuration)}
** Chiffrer les mots de passe
Pour *chiffrer* tous les mots de passe en texte clair, il faut utiliser les commandes suivantes:
- =configure terminal= \textcolor{luckydarkgray}{(Entrer dans le mode de configuration globale)}
- =service password-encryption= \textcolor{luckydarkgray}{(Activation du chiffrement des mots de passe non chiffrés)}
...
- =erase startup-config= \textcolor{luckydarkgray}{(Supprimer le fichier "startu-config")}
...
- *Exécuter* la commande "=show running-config=" ou "=show startup-config=" dans l'invité de commande d'exécution privilégiée \textcolor{luckydarkgray}{(Le texte affiché dans le fenêtre du terminal est alor placé dans le fichier configuré à la deuxième étape)}
